...
固定 STT 配置中的 deepinfra、azure 等值是目录预检所要求的 endpoint 预期值，不是 STT 请求携带的 pin，也不能冒充响应已经确认的实际 endpoint。只有 STT 响应明确返回的 provider 才能记为 reported provider；未报告时保持 unreported，不用 expected 值补成 actual。自动路由同样不会把字符串 any 冒充实际 endpoint。
...
quality_asr_model = "microsoft/mai-transcribe-2"
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 3.7 quality（当时默认） | 0.000000 | 全部 1.000000 | 1.000000 | 17.665 s | 4 | $0.003259811 |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| 3.7 quality（当时默认） | 0.000000 | 1.000000 | 93.986 s | 4 | $0.003443490 |
